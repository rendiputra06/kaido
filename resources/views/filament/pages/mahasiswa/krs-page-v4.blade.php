                ...
                <div class="flex flex-wrap items-center justify-between gap-4">
                    ...
                    <div class="flex items-center space-x-4">
                        <div class="text-center">
                            ...
                            <span
                                class="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold {{ $this->krs?->status->getColor() ?? 'bg-gray-100 text-gray-800' }}">
                                ...
                                <div class="space-y-2">
                                    @if($this->krs->status->value === 'draft')
                                        <button wire:click="submitKrs"
                                                class="bg-blue-600 hover:bg-blue-700 text-black font-semibold py-2 px-4 rounded-lg
                                                shadow-md transition-all duration-150 ease-in-out focus:outline-none focus:ring-2
                                                focus:ring-offset-2 focus:ring-blue-500 hover:scale-105 active:scale-95 active:shadow-inner">
                                            Submit KRS
                                        </button>
                                    @elseif($this->krs->status->value === 'submitted')
                                        <button wire:click="cancelSubmit"
                                                class="bg-yellow-500 hover:bg-yellow-600 text-black font-semibold py-2 px-4 rounded-lg
                                                shadow-md transition-all duration-150 ease-in-out focus:outline-none focus:ring-2
                                                focus:ring-offset-2 focus:ring-yellow-500 hover:scale-105 active:scale-95 active:shadow-inner">
                                            Batalkan Submit
                                        </button>
                ...
                <div class="bg-white rounded-xl shadow-md border border-gray-200 flex flex-col">
                    ...
                    <div class="p-4 space-y-3 overflow-y-auto hide-scrollbar" style="max-height: 60vh;">
                        @forelse($this->availableClasses as $kelas)
                            <div class="border rounded-lg transition-all {{ $kelas['is_taken'] ? 'bg-green-50 border-green-200' : 'hover:border-blue-400 hover:shadow-sm' }}">
                                <div class="p-3">
                                    <div class="flex items-start justify-between">
                                            ...
                                            <div class="mt-2 space-y-1">
                                                @foreach($kelas['jadwal'] as $jadwal)
                                                    <div class="flex items-center text-xs text-gray-600">
                                                        ...
                                                        <span class="ml-1">{{ $jadwal['jam_mulai'] }} - {{ $jadwal['jam_selesai'] }}</span>
                                                        <span class="ml-2 px-1 py-0.5 bg-gray-100 rounded text-xs">{{ $jadwal['ruang'] }}</span>
                                                    </div>
                                                @endforeach
                                            </div>
                                        </div>
                                        <div class="flex items-center space-x-2 ml-2">
                                            ...
                                            @elseif(!$this->activePeriod || ($this->krs && $this->krs->status->value !== 'draft'))
                                                <button disabled
                                                        class="p-2 bg-gray-200 text-gray-400 rounded-full cursor-not-allowed">
                                            ...
                                            @else
                                                <button wire:click="addClass({{ $kelas['id'] }})"
                                                        class="p-2 bg-blue-100 text-blue-600 hover:bg-blue-200 rounded-full transition-transform hover:scale-110">
                                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                              d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                                    </svg>
                                        ...
                                        <div class="flex items-start justify-between">
                                            <div class="flex-grow">
                                                <h3 class="font-semibold text-gray-800">
                                                    {{ $detail->kelas->mataKuliah->nama_mk }}</h3>
                                                <p class="text-xs text-gray-500">{{ $detail->kelas->dosen->nama }}</p>
                                                <p class="text-xs text-gray-500">{{ $detail->kelas->mataKuliah->sks }} SKS</p>
                                                ...
                                                <div class="mt-2 space-y-1">
                                                    @foreach($detail->kelas->jadwalKuliahs as $jadwal)
                                                        <div class="flex items-center text-xs text-gray-600">
                                                            ...
                                                            <span class="font-medium">{{ $jadwal->hari }}:</span>
                                                            <span class="ml-1">{{ date('H:i', strtotime($jadwal->jam_mulai)) }} - {{ date('H:i', strtotime($jadwal->jam_selesai)) }}</span>
                                                            <span class="ml-2 px-1 py-0.5 bg-white rounded text-xs border">{{ $jadwal->ruangKuliah->nama_ruang }}</span>
                                                        </div>
                                                    @endforeach
                                                </div>
                                            </div>
                                            <div class="flex items-center space-x-2 ml-2">
                                                @if($this->krs->status->value === 'draft')
                                                    <button wire:click="removeClass({{ $detail->id }})"
                                                            class="p-2 bg-red-100 text-red-600 hover:bg-red-200 rounded-full transition-transform hover:scale-110">
                                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <div class="text-center py-10 flex flex-col items-center justify-center h-full">
                                <div class="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mb-4">
                                    <svg class="w-8 h-8 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                              d="M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0-2.21 3.582-4 8-4s8 1.79 8 4M4 7s-1 1-1 4m18-4s1 1 1 4"/>
                                    </svg>
                                </div>
