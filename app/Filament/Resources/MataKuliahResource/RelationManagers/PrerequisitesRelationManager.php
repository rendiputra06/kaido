...
{
    protected static string $relationship = 'prasyarats';

    protected static ?string $recordTitleAttribute = 'nama_mk';
    
    protected static ?string $title = 'Mata Kuliah Prasyarat';
    
    protected static ?string $label = 'Mata Kuliah Prasyarat';
    
    protected static ?string $modelLabel = 'Mata Kuliah Prasyarat';
            ...
            ->schema([
                Forms\Components\Select::make('prasyarat_id')
                    ...
                    ->options(function (RelationManager $livewire): array {
                        ...
                        // Ambil semua mata kuliah kecuali yang saat ini dan yang sudah menjadi prasyarat
                        return MataKuliah::query()
                            ->where('id', '!=', $currentMataKuliahId)
                            ->whereDoesntHave('mataKuliahLanjutan', function (Builder $query) use ($currentMataKuliahId) {
                                $query->where('mata_kuliah_id', $currentMataKuliahId);
                            })
                            ->pluck('nama_mk', 'id')
                            ->toArray();
                    })
        ...
        return $table
            ...
            ->headerActions([
                Tables\Actions\CreateAction::make()
                    ...
                    ->modalHeading('Tambah Mata Kuliah Prasyarat'),
            ])
            ->actions([
                Tables\Actions\DeleteAction::make()
                    ->label('Hapus Prasyarat'),
            ])
            ->bulkActions([
